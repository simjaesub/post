...
def matgen(degree, failure):
    with open('..\\..\\..\\OneDrive - Universitetet i Stavanger\\Fhsim_wake\\bin\\Results_singlecage_failure\\' + str(
            failure) + '_fail\\dprocessed_Mul1x1Vel0.5Degree' + str(
        degree) + '_' + str(failure) + '_fail.csv', 'rb') as handle:
        resuF = pickle.load(handle)

    with open(
            '..\\..\\..\\OneDrive - Universitetet i Stavanger\\Fhsim_wake\\bin\\Results_singlecage_intact_d\\dprocessed_Mul1x1Vel0.5Degree' + str(
                degree) + '.csv',
            'rb') as handle:
    ...
    for i in range(len(bridles_intact[0, :])):
    ...
    mat[fmodes.index(failure)][
        len(anchors) + 1 + len(fcables) + 1 + len(bridles_fail[0, :]) + 1] = vol_intact - vol_fail
    ...
    mat[fmodes.index(failure)][
        len(anchors) + 1 + len(fcables) + 1 + len(bridles_fail[0, :]) + 1 + 1 + 1] = drag_intact - drag_fail
    ...
    for i in range(len(B_pos)):
        mat[fmodes.index(failure)][
            len(anchors) + 1 + len(fcables) + 1 + len(bridles_fail[0, :]) + 1 + 1 + 1 + 1 + 1 + i] = B_pos[i][0]
